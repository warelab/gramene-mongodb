...
function getMapping() {
  ...
  var lut = {};
  var filename = 'geneIDhistory.maizeV5';
  console.error('fixMaizeV5 getMapping');
...
function getNames() {
  var deferred = Q.defer();
  var lut = {};
  var filename = 'maize_gene_info.txt';
  console.error('fixMaizeV5 getNames');
  require('readline').createInterface({
    input: require('fs').createReadStream(filename),
    terminal: false
  })
  ...
  .on('close', function() {
    console.error('loaded names lut');
    deferred.resolve(lut);
  });
  return deferred.promise;
}
      ...
      if (lut[gene._id]) {
        ...
        if (!gene.description && v3descriptions.length == 1) {
          gene.description = v3descriptions[0];
			...
			namesPromise.then(function(lut2) {
				if (lut2[gene._id]) {
					gene.name = lut2[gene._id].name;
					gene.description = lut2[gene._id].desc || "";
				}
				else {
